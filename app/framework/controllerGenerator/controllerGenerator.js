...
exports.generateControllerFile = async (controllerName) => {
...
exports.no_db_controller = async (controllerName) => {
    const json = path.join(__dirname, '..', 'ProjectInitiator', 'config.json');
    const content = await fs.readFile(json, 'utf8');
    const config = JSON.parse(content);
    ...
    if (is_configure) {
        ...
        try {
            const templatePath = path.join(__dirname, 'no_db_controller_template.txt');
            templateContent = await fs.readFile(templatePath, 'utf8');
        }
        catch (error) {
            console.error(`${color.red}[X]------- Failed to read template file: ${error.message}${color.default_color}`);
